...
BOOST_AUTO_TEST_CASE(should_return_empty_string_from_empty_string)
{
	BOOST_CHECK(VerifyHtmlDecode("", ""));
}

BOOST_AUTO_TEST_CASE(should_not_change_string_without_html_entities)
{
	BOOST_CHECK(VerifyHtmlDecode("The quick brown fox jumps over the lazy dog", 
				     "The quick brown fox jumps over the lazy dog"));
}

BOOST_AUTO_TEST_CASE(should_replaces_each_html_entity_with_corresponding_character)
{
	BOOST_CHECK(VerifyHtmlDecode("Cat &lt;says&gt; &quot;Meow. M&amp;M&apos;s", 
				     "Cat <says> \"Meow. M&M's"));
}

BOOST_AUTO_TEST_CASE(should_correctly_replaces_recursived_html_entities)
{
	BOOST_CHECK(VerifyHtmlDecode(
